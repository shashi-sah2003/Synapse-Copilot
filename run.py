...
def main():
    ...
    if scenario == "tmdb":
        ...
        api_spec, headers = process_spec_file(
            file_path="specs/notion_oas.json", token=os.environ["NOTION_TOKEN"]
        )

        query_example = "Get me my details"
    ...
    llm = Ollama(model="llama3")
    #llm = pipeline("text-generation", model="TinyLlama/TinyLlama-1.1B-Chat-v1.0", tokenizer="TinyLlama/TinyLlama-1.1B-Chat-v1.0", max_length=1024)
    api_llm = ApiLLM(
        llm,
        api_spec=api_spec,
        scenario=scenario,
    ...
    # # Using Ollama's generate method
    # response = ollama.generate(model='llama3', prompt=query)

    # print(f"Response: {response['messages']['content']}")
